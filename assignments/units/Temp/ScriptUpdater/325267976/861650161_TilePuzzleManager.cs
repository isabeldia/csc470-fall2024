...
public class TilePuzzleManager : MonoBehaviour
{
    ...
    // get; private set; allows reading but modifications can only be made in this class; fixes error CS0122
    ...
    public string CurrentFlavor => currentFlavor;
    ...
    public void ProcessTileEffect(Vector2Int position, Vector3 currentWorldPos)
    {
        Debug.Log($"Processing Tile Effect at position {position}, Current Pos: {currentWorldPos}");
        
        if (isProcessingEffect) 
        {
            Debug.Log("Already processing an effect, returning");
            return;
        }
        
        isProcessingEffect = true;

        previousPosition = currentWorldPos;
        
        TileType currentTile = puzzleGrid[position.x, position.y];
        Debug.Log($"Current Tile Type: {currentTile}");
        
        switch (currentTile)
        {
            case TileType.Pink:
                Debug.Log("Pink Tile: No effect");
    ...
    {
        Debug.Log($"Current Flavor: {currentFlavor}");
    ...
    {
        Debug.Log($"Yellow Tile: Moving back to previous position {previousPosition}");
        
        player.MoveToWorldPosition(previousPosition, () => {
    ...
    private void HandleGreenTile()
    {
        ...
        player.MoveToWorldPosition(startPosition, () => {
            ...
            isProcessingEffect = false;
            // Update previous position to start position
            previousPosition = startPosition;
        });
    }
    ...
    private void HandleBlueTile(Vector2Int position)
    {
        ...
        // Conditions for pushing back
        if (adjacentToYellow || currentFlavor == "Orange")
        {
            Debug.Log("Blue Tile pushing back due to yellow adjacency or orange flavor");
            HandleYellowTile();
        }
        else
        {
            Debug.Log("Blue Tile has no special effect");
            isProcessingEffect = false;
        }
    }
...
public class PlayerController : MonoBehaviour
{
    ...
    {
        Debug.Log($"Moving to world position: {targetPos}");
        
        if (isMoving) 
        {
            Debug.Log("Already moving, cannot move again");
            return;
        }
    ...
    private void TryMove(Vector2Int movement)
    {
        ...
        Debug.Log($"Attempting to move from {currentPos} to {targetPos}");
    ...
    void Update()
    {
        ...
        if (currentState == GameState.PuzzleMode)
        {
            HandlePuzzleMovement();
        }
        else if (currentState == GameState.RaycastMode)
        {
    ...
    private void HandleInput()
    {
        ...
        if (Input.GetKeyDown(KeyCode.W)) 
        {
            ...
            {
                Debug.Log("Reached top row! Switching to raycast mode...");
