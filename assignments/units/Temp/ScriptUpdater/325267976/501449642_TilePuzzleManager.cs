...
public class TilePuzzleManager : MonoBehaviour
{
    ...
    public int gridHeight { get; private set; } = 8;  // changed to property w/ initial value
...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleRaycastMovement()
    {
        if (!navAgent.enabled)
        {
            ...
            Debug.Log("NavMeshAgent enabled and configured");
        }
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, interactableLayerMask))
            {
                ...
                else if (isSelected)
                {
                    if (UnityEngine.AI.NavMesh.SamplePosition(hit.point, out UnityEngine.AI.NavMeshHit navHit, 5f, UnityEngine.AI.NavMesh.AllAreas))
                    {
                        ...
                        Debug.Log($"Setting NavMesh destination to: {navHit.position}");
                    }
                    else
                    {
                ...
                {
                    Debug.Log("Hit something but player not selected");
                }
            }
            else
            {
        ...
        if (navAgent.hasPath)
        {
            Debug.Log($"Moving to destination. Remaining distance: {navAgent.remainingDistance}");
        }
    }
